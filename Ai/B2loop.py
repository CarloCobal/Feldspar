...
def historical_parser():
    try:
        ...
        if not history:
            logging.info("Search history is empty, using default value.")
            # If you want to treat empty history as an empty string
            return ""

            # If you want to treat the most recent query as the entire history
            # return "Your most recent query"  # Replace with the actual most recent query if available

        return ', '.join(f"{index + 1}. {item}" for index, item in enumerate(history))

    except FileNotFoundError:
        logging.error("No history file found in UserDat/searchHistory.txt")
        return "Error: Search history file not found."
    except Exception as e:
        logging.error(f"Error reading history file: {e}")
        return "Error: Unexpected issue with search history file."
