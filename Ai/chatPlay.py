...
def call1(user_prompt, intake):
    """Intake prompt with adjustable bias"""
...
question2= "Given the following text, please find one relevant searchable questions that I can ask google. No other text is needed."
...
async def process_output(output, action):
    """Loops through chat until a google-searchable query is formed and then performs a Google search."""
    while True:
        if action == "chat":
...
async def main():
    ...
    for q2_output, decision in zip(question2_outputs, question3_decisions):
        task = asyncio.create_task(process_output(q2_output.strip(), decision.strip().lower()))
        tasks.append(task)
